...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (loggedInUser) {
      ...
      setIsLogin({ username: foundUser.username, nickname: foundUser.nickname });
    }
  }, []);
  ...
  const handleLogin = (e) => {
    ...
    try {
      ...
      if (!user) {
        ...
        setIsLogin({ username });  // 更新父層狀態
        localStorage.setItem('user', JSON.stringify({ username }));  // 保存登入狀態到 localStorage
      }
    } catch (error) {
  ...
  // 登出
  const handleLogout = () => {
    setIsLogin({ username: '' });    // 清除狀態
    // localStorage.removeItem('isLoggedIn');      // 從 localStorage 中移除登入狀態
    localStorage.removeItem('user');      // 從 localStorage 中移除登入狀態
  };
