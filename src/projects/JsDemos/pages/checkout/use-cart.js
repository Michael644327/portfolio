...
//    目的: 提供給最上層元件(_app.js)方便使用，共享狀態在此統一管理
export function CartProvider({ children }) {
  //children指以下被包覆的所有子女元件
  ...
  // 用陣列迭代方式reduce計算總金額、數量
  const totalQty = items.reduce((acc, v) => acc + v.qty, 0)
  totalPrice = items.reduce((acc, v) => acc + v.qty * v.price, 0)

  return (
    <CartContext.Provider
      // 使用value屬性，提供資料給元件階層以下的所有後代元件(for consumer)
      value={{
        items,
        totalQty,
        totalPrice,
